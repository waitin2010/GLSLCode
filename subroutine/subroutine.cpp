...
void init()
{
	shader = new GLShader("shader/subroutine.vert","shader/subroutine.frag");
	model = new Obj();
	char dir[256];
...
void display()
{
	...
	shader->begin();
	GLuint adsIndex, diffuseIndex;
	adsIndex = glGetSubroutineIndex(shader->getProgram(),GL_VERTEX_SHADER,"phoneModel");
	diffuseIndex = glGetSubroutineIndex(shader->getProgram(),GL_VERTEX_SHADER,"diffuseModel");
	...
	glm::vec3 lightPos = glm::vec3(10.0,0.0,10.0);
	...
	model_matrix = glm::translate(model_matrix,glm::vec3(-1.0,-1.0,0));
	...
	model_matrix = glm::rotate(glm::mat4(1.0),angle,glm::vec3(0.0,1.0,0.0));
	model_matrix = glm::translate(model_matrix,glm::vec3(1.0,-1.0,0));
	shader->setUniform("model_matrix",model_matrix);
