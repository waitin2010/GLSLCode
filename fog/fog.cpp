...
void init()
{
	shader = new GLShader("shader/fog.vert","shader/fog.frag");
	model = new Obj();
	char dir[256];
...
void display()
{
	...
	// for fog
	shader->setUniform("fog.distMin",distMin);
	shader->setUniform("fog.distMax",distMax);
	...
	model_matrix = glm::translate(mat4(1.0),vec3(1.0,-1.0,-2.0));
	shader->setUniform("model_matrix",model_matrix);
	model->draw();

	model_matrix = glm::translate(mat4(1.0),vec3(2.0,-1.0,-4.0));
	shader->setUniform("model_matrix",model_matrix);
	model->draw();
