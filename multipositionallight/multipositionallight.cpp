...
void init()
{
	shader = new GLShader("shader/multipositionallight.vert","shader/multipositionallight.frag");
	model = new Obj();
	char dir[256];
...
void display()
{
	...
	glm::vec3 lightPos2 = vec3(-2.0,0.0,0.0);
	...
	// for light
	shader->setUniform("mLight[0].position",lightPos);
	shader->setUniform("mLight[0].La",La);
	shader->setUniform("mLight[0].Ld",Ld);
	shader->setUniform("mLight[0].Ls",Ls);

	shader->setUniform("mLight[1].position",lightPos2);
	shader->setUniform("mLight[1].La",La);
	shader->setUniform("mLight[1].Ld",Ld);
	shader->setUniform("mLight[1].Ls",Ls);
	...
	model_matrix = glm::translate(glm::mat4(1.0),lightPos2);
	shader->setUniform("model_matrix",model_matrix);
