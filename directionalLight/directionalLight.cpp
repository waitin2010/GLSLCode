...
void init()
{
	shader = new GLShader("shader/directionalLight.vert","shader/directionalLight.frag");
	model = new Obj();
	char dir[256];
...
void display()
{
	...
	glm::vec4 lightPos = glm::vec4(2.0,0.0,0.0,0.0);
	...
	model_matrix = glm::translate(glm::mat4(1.0),vec3(lightPos));
	shader->setUniform("model_matrix",model_matrix);
