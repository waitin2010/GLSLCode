...
vec3 phoneModel(int lightIndex, in vec3 pos, in vec3 N)
{
	LightInfo light = mLight[lightIndex];
...
void main()
{
	...
	for(int i=0; i<2; i++)
	{
		frontColor += vec4(phoneModel(i,pos,N),1.0);
		backColor += vec4(phoneModel(i,pos,-N),1.0);
	}
